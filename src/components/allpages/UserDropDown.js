...
function UserDropDown({ teacherProfile, learnerProfile, userAccount }) {
  const { user } = useContext(AuthContext);
  const { setShowTeacherForm } = useContext(TeacherFormContext)
  const { setShowAvailableChoose, setShowLessonForm, setIsEditLesson } = useContext(ModalContext)
  const handleLogout = () => {
    removeToken();
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-body">
              <div className="d-flex justify-content-center align-items-center mb-4 ms-5 mt-3">
                <div className="d-flex justify-content-center align-items-center">
                  ...
                  <div className='d-flex flex-column '>
                    <p className='badge bg-warning text-black text-wrap m-0 p-1'>{userAccount.studentPoint} XP</p>
              ...
              {user.typeAccount === 'learner' && learnerProfile && <Link to={`/learnProfile/${user.id}`}><p onClick={() => setTimeout(() => { window.location.reload() }, 1000)} className='btn btn-secondary form-control' data-bs-dismiss="modal">My Profile</p></Link>}
              {user.typeAccount === 'teacher' && teacherProfile && <Link to={`/teacherProfile/${teacherProfile?.id}`}><p onClick={() => setTimeout(() => { window.location.reload() }, 1000)} className='btn btn-secondary form-control' data-bs-dismiss="modal">My Profile</p></Link>}
              ...
              {user.typeAccount === 'teacher' && <p className='btn btn-secondary form-control' onClick={() => setShowAvailableChoose(true)}>Available Time Setting</p>}
